...
function SmoothZoomedCamera:__newindex(index, value)
	...
	elseif index == "Value" or index == "Zoom" then
		self.Spring:SetPosition(math.clamp(value, self.MinZoom, self.MaxZoom))
...
function SmoothZoomedCamera:__tostring()
	return "SmoothZoomedCamera"
end

table.freeze(SmoothZoomedCamera)
return SmoothZoomedCamera
