...
function SummedCamera:__index(index)
	if index == "CameraState" then
		if self._mode == "World" then
			...
			local newQFrameVelocity = QFrame.FromCFrameClosestTo(a.CameraFrameDerivative.CFrame * b.CameraFrameDerivative.CFrame, a.CameraFrameDerivative.QFrame)
			local cameraFrameVelocity = CameraFrame.new(newQFrameVelocity, a.CameraFrameDerivative.FieldOfView + b.CameraFrameDerivative.FieldOfView)
	...
	elseif SummedCamera[index] then
		...
		Debug.Error("[SummedCamera] - %q is not a valid member", index)
	end
end

function SummedCamera:__tostring()
	return "SummedCamera"
end

table.freeze(SummedCamera)
return SummedCamera
