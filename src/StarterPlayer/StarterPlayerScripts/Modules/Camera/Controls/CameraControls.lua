...
local GamepadRotateModel = require(script.Parent.GamepadRotateModel)
local Janitor = require(ReplicatedStorage.Knit.Util.Janitor)
local InputObjectUtility = require(StarterPlayerScripts:WaitForChild("Modules"):WaitForChild("Utility"):WaitForChild("InputObjectUtility"))
...
function CameraControls:Enable()
	...
	assert(not self._janitor, "Janitor already defined")
	self._enabled = true

	self._janitor = Janitor.new()

	self._janitor:Add(self._gamepadRotateModel.IsRotating.Changed:Connect(function()
	...
	end, false, table.unpack(self._dragBeginTypes))
	...
	self._janitor:Add(UserInputService.TouchPinch:Connect(function(_, scale, velocity, userInputState)
...
function CameraControls:BeginDrag(beginInputObject)
	if not self._rotatedCamera then
		self._janitor:Remove("DragJanitor")
		return
	end

	local dragJanitor = self._janitor:Add(Janitor.new(), "Destroy", "DragJanitor")

	self._lastMousePosition = beginInputObject.Position
	local isMouse = InputObjectUtility.IsMouseUserInputType(beginInputObject.UserInputType)
	if isMouse then
	...
	dragJanitor:Add(UserInputService.InputChanged:Connect(function(inputObject)
		if InputObjectUtility.IsMouseUserInputType(inputObject.UserInputType) or inputObject == beginInputObject then
...
function CameraControls:_handleMouseMovement(inputObject)
	if self._lastMousePosition then
		if self._rotatedCamera then
			...
			local delta
			if InputObjectUtility.IsMouseUserInputType(inputObject.UserInputType) then
...
function CameraControls:__tostring()
	return "CameraControls"
end

table.freeze(CameraControls)
return CameraControls
