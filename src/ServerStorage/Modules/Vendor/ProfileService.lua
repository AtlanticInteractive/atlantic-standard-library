...
-[ProfileService]---------------------------------------
	...
	Methods [Profile]:
		...
		Profile:RemoveUserId(user_id) -- Disassociates user_id with profile (safe function)
			user_id   [number]
...
do
	...
	Madwork = {
		...
		ConnectToOnClose = function(task, run_in_studio_mode)
			if RunService:IsStudio() == false or run_in_studio_mode == true then
...
ProfileService.ServiceLocked = false -- Set to true once the server is shutting down
...
local function IdentifyProfile(store_name, store_scope, key)
	return string.format("[Store:%q;%sKey:%q]", store_name, store_scope ~= nil and string.format("Scope:%q;", store_scope) or "", key)
end
...
local function MockUpdateAsync(mock_data_store, profile_store_name, key, transform_function, is_get_call) --> loaded_data, key_info
	...
	local epoch_time = math.floor(DateTime.now().UnixTimestamp * 1000)
			...
			EditProfile = function(latest_data)
				...
				if session_owns_profile == true then -- We may only edit the profile if this session has ownership of the profile
					if is_overwriting ~= true then
						...
						for _, latest_global_update in ipairs(latest_global_updates_list) do
						...
						for _, clear_id in ipairs(pending_update_clear) do
							for i, latest_global_update in ipairs(latest_global_updates_list) do
					...
					if is_overwriting ~= true then
						...
						latest_data.Metadata.LastUpdate = DateTime.now().UnixTimestamp
...
function Profile:StoreOnAttributeChange(Name: string, Object: Instance)
	if self.UsedKeys[Name] then
		error(string.format("[Profile.StoreOnAttributeChange] - Already have a writer for %q", Name), 2)
	end

	self.UsedKeys[Name] = true
	return self.Janitor:Add(Object:GetAttributeChangedSignal(Name):Connect(function()
...
function Profile:ListenToRelease(listener) --> [ScriptConnection] (place_id / nil, game_job_id / nil)
	...
	if self:IsActive() == false then
		...
		return self.Janitor:Add(self._release_listeners:Connect(listener), "Disconnect")
	end
end
...
function Profile:Save()
	...
	--	unnecessary API request spam which we could not meaningfully execute anyways!
...
function Profile:ListenToHopReady(listener) --> [ScriptConnection] ()
	...
	if self._hop_ready == true then
		...
		return self.Janitor:Add(self._hop_ready_listeners:Connect(listener), "Disconnect")
	end
end
...
function Profile:AddUserId(user_id) -- Associates user_id with profile (GDPR compliance)
	if type(user_id) ~= "number" or user_id % 1 ~= 0 then
		return warn("[ProfileService]: Invalid UserId argument for :AddUserId() (" .. tostring(user_id) .. "); Traceback:\n" .. debug.traceback())
	end
...
function ProfileStore:LoadProfile(profile_key, not_released_handler, _use_mock) --> [Profile / nil] not_released_handler(place_id, game_job_id)
	not_released_handler = not_released_handler or Enumeration.DataStoreHandler.ForceLoad
	...
	if type(not_released_handler) ~= "function" then
		not_released_handler = assert(Enumeration.DataStoreHandler:Cast(not_released_handler))
	end
	...
	while ProfileService.ServiceLocked == false do
		...
		-- grab the DataStore return for the new call. The older call will return nil. This would prevent very rare
		...
		if profile_load_job ~= nil then
			profile_load_job[1] = load_id -- Grab load job
			while profile_load_job[2] == nil do -- Wait for job to finish
				task.wait()
			end

			if profile_load_job[1] == load_id then -- Load job hasn't been double-grabbed
			...
			profile_load_job[2] = table.pack(StandardProfileUpdateAsyncDataStore(self, profile_key, {
						...
						elseif type(active_session) == "table" then
							if IsThisSession(active_session) == false then
								...
								if last_update ~= nil then
									if DateTime.now().UnixTimestamp - last_update > SETTINGS.AssumeDeadSessionLock then
										latest_data.Metadata.ActiveSession = {PlaceId, JobId}
										latest_data.Metadata.ForceLoadSession = nil
										return
									end
				...
				MissingProfileHandle = function(latest_data)
					...
					latest_data.Metadata = {
						ProfileCreateTime = DateTime.now().UnixTimestamp;
						SessionLoadCount = 0;
			...
			if profile_load_job[1] == load_id then -- Load job hasn't been grabbed
				loaded_data, key_info = table.unpack(profile_load_job[2])
				profile_load_jobs[profile_key] = nil
			else
				ActiveProfileLoadJobs -= 1
			...
			if type(active_session) == "table" then
				if IsThisSession(active_session) == true then
					...
					elseif aggressive_steal == true then
						...
						local handler_result = assert(Enumeration.DataStoreHandler:Cast(not_released_handler(active_session[1], active_session[2])))
	...
	profile_store = {
		Mock = {
			...
			FindProfileVersion = function(_, profile_key, sort_direction, min_date, max_date)
				return profile_store:FindProfileVersion(profile_key, sort_direction, min_date, max_date, UseMockTag)
			end;
		};
...
if IsStudio == true then
	...
	task.spawn(function()
		local status, message = pcall(function()
			...
			DataStoreService:GetDataStore("____PS"):SetAsync("____PS", DateTime.now().UnixTimestamp)
		end)
