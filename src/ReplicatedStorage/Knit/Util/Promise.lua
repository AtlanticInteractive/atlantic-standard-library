...
local function Promise_new(traceback, callback, parent)
	...
	if parent and parent.Status == Promise.Status.Started then
	...
	local function onCancel(cancellationHook)
		if cancellationHook then
			if self.Status == Promise.Status.Cancelled then
				cancellationHook()
			else
		...
		return self.Status == Promise.Status.Cancelled
	end
...
function Promise.new(executor)
	return Promise_new(debug.traceback(nil :: any, 2), executor or NoOp)
end

function Promise:__tostring()
	return string.format("Promise(%s)", self.Status)
end
...
function Promise.Defer(executor)
	...
	return Promise_new(traceback, function(resolve, reject, onCancel)
		task.defer(function()
...
function Promise.Resolve(...)
	...
	return Promise_new(debug.traceback(nil :: any, 2), function(resolve)
...
function Promise.Reject(...)
	local values = table.pack(...)
	return Promise_new(debug.traceback(nil :: any, 2), function(_, reject)
...
local function Promise_try(traceback, callback, ...)
	local values = table.pack(...)

	return Promise_new(traceback, function(resolve)
...
function Promise.Try(callback, ...)
	return Promise_try(debug.traceback(nil :: any, 2), callback, ...)
end
...
local function Promise_all(traceback, promises, amount)
...
function Promise.All(promises)
	return Promise_all(debug.traceback(nil :: any, 2), promises)
end
...
function Promise.Fold(list, reducer, initialValue)
	...
	return Promise.Each(list, function(resolvedElement, i)
		accumulator = accumulator:Then(function(previousValueResolved)
			return reducer(previousValueResolved, resolvedElement, i)
		end)
	end):ThenReturn(accumulator)
end
...
function Promise.Some(promises, count)
	...
	return Promise_all(debug.traceback(nil :: any, 2), promises, count)
end
...
function Promise.Any(promises)
	return Promise_all(debug.traceback(nil :: any, 2), promises, 1):Then(function(values)
		return values[1]
	end)
end
...
function Promise.AllSettled(promises)
	...
	return Promise_new(debug.traceback(nil :: any, 2), function(resolve, _, onCancel)
		-- An array to contain our resolved values from the given promises.
...
function Promise.Race(promises)
	...
	return Promise_new(debug.traceback(nil :: any, 2), function(resolve, reject, onCancel)
		local newPromises = {}
	...
	return Promise_new(debug.traceback(nil :: any, 2), function(resolve, reject, onCancel)
		...
		for index, value in ipairs(list) do
			if Promise.Is(value) then
				if value.Status == Promise.Status.Cancelled then
					cancel()
				...
				elseif value.Status == Promise.Status.Rejected then
					cancel()
...
function Promise.Is(object)
	...
	elseif objectMetatable == nil then
		...
		return isCallable(object.Then)
	elseif type(objectMetatable) == "table" and type(rawget(objectMetatable, "__index")) == "table" and isCallable(rawget(rawget(objectMetatable, "__index"), "Then")) then
...
function Promise.prototype:Timeout(seconds, rejectionValue)
	...
	return Promise.Race({
		Promise.Delay(seconds):Then(function()
...
local function AndThen(self, traceback, successHandler, failureHandler, useSpread)
	...
	if useSpread then
		if successHandler then
			...
			function successHandler(data)
				if type(data) == "table" and data._fromAll then
	...
	return Promise_new(traceback, function(resolve, reject, onCancel)
		...
		if self.Status == Promise.Status.Started then
		...
		elseif self.Status == Promise.Status.Resolved then
...
function Promise.prototype:Then(successHandler, failureHandler)
	...
	return AndThen(self, debug.traceback(nil :: any, 2), successHandler, failureHandler, false)
end

function Promise.prototype:Spread(successHandler)
	assert(successHandler == nil or isCallable(successHandler), string.format(ERROR_NON_FUNCTION, "Promise:spread"))
	return AndThen(self, debug.traceback(nil :: any, 2), successHandler, nil, true)
end

local function ConsumerCancelled(self, consumer)
	if self.Status ~= Promise.Status.Started then
		return
	end
...
function Promise.prototype:Catch(failureHandler)
	...
	return AndThen(self, debug.traceback(nil :: any, 2), nil, failureHandler, false)
end
...
function Promise.prototype:Tap(tapCallback, tapCatch)
	...
	assert(tapCatch == nil or isCallable(tapCatch), string.format(ERROR_NON_FUNCTION, "Promise:tap"))
	tapCallback = createTapper(tapCallback)

	if tapCatch then
		tapCatch = createTapper(tapCatch)
		return AndThen(self, debug.traceback(nil :: any, 2), tapCallback, tapCatch, false)
	else
		return AndThen(self, debug.traceback(nil :: any, 2), tapCallback, nil, false)
	end
end
...
function Promise.prototype:ThenCall(callback, ...)
	...
	local values = table.pack(...)
	return AndThen(self, debug.traceback(nil :: any, 2), function()
...
local function Finalize(self)
	for _, callback in ipairs(self._queuedFinally) do
		...
		task.spawn(callback, self.Status)
	end
...
function Promise.prototype:Cancel()
	...
	self.Status = Promise.Status.Cancelled
	...
	if self._parent then
		ConsumerCancelled(self._parent, self)
	end
...
local function Finally(self, traceback, finallyHandler)
	self._unhandledRejection = false

	local promise = Promise_new(traceback, function(resolve, reject, onCancel)
...
function Promise.prototype:Finally(finallyHandler)
	...
	return Finally(self, debug.traceback(nil :: any, 2), finallyHandler)
end
...
function Promise.prototype:FinallyCall(callback, ...)
	...
	local values = table.pack(...)
	return Finally(self, debug.traceback(nil :: any, 2), function()
		return callback(table.unpack(values, 1, values.n))
	end)
end
...
function Promise.prototype:WaitStatus()
	...
	if self.Status == Promise.Status.Resolved then
		return self.Status, table.unpack(values, 1, values.n)
	elseif self.Status == Promise.Status.Rejected then
		return self.Status, table.unpack(values, 1, values.n)
	end
...
function Promise.prototype:_Unwrap()
	...
	local success = self.Status == Promise.Status.Resolved
	local values = self._values
...
function Promise.prototype:Resolve(...)
	...
	if Promise.Is((...)) then
		...
		if promise.Status == Promise.Status.Cancelled then
			self:Cancel()
		elseif promise.Status == Promise.Status.Started then
	...
	if not isEmpty(self._queuedReject) then
		...
		task.defer(function()
			...
			for _, callback in ipairs(Promise._unhandledRejectionCallbacks) do
				task.spawn(callback, self, table.unpack(values, 1, values.n))
			end
...
function Promise.prototype:Now(rejectionValue)
	local traceback = debug.traceback(nil :: any, 2)
	if self.Status == Promise.Status.Resolved then
		return AndThen(self, traceback, function(...)
			return ...
		end)
	else
		return Promise.Reject(rejectionValue == nil and Error.new({
...
function Promise.RetryWithDelay(callback, times, seconds, ...)
	...
	return Promise.Resolve(callback(...)):Catch(function(...)
		if times > 0 then
			Promise.Delay(seconds):Wait()

			return Promise.RetryWithDelay(callback, times - 1, seconds, table.unpack(args, 1, args.n))
		else
			return Promise.Reject(...)
		end
	end)
...
Promise.prototype.AndThen = Promise.prototype.Then
Promise.prototype.andThen = Promise.prototype.Then
Promise.prototype.AndThenCall = Promise.prototype.ThenCall
Promise.prototype.andThenCall = Promise.prototype.ThenCall
Promise.prototype.AndThenReturn = Promise.prototype.ThenReturn
Promise.prototype.andThenReturn = Promise.prototype.ThenReturn
Promise.prototype.Await = Promise.prototype.Wait
Promise.prototype.await = Promise.prototype.Wait
Promise.prototype.AwaitStatus = Promise.prototype.WaitStatus
Promise.prototype.awaitStatus = Promise.prototype.WaitStatus
...
Promise.prototype.wait = Promise.prototype.Wait
Promise.prototype.waitStatus = Promise.prototype.WaitStatus
