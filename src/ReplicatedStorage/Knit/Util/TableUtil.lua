...
local rng = Random.new(os.clock() % 1 * 1E7)
...
local function Copy(t: Table, deep: boolean?): Table
	if deep then
		...
		local length = #t
		if length > 0 then
			return table.move(t, 1, length, 1, table.create(length)) :: any
		else
			local tCopy = {}
...
local function Sync(srcTbl: Table, templateTbl: Table): Table
	Debug.Assert(type(srcTbl) == "table", "First argument must be a table; got %q", srcTbl)
	Debug.Assert(type(templateTbl) == "table", "Second argument must be a table; got %q", templateTbl)
...
local function Reconcile(src: Table, template: Table): Table
	Debug.Assert(type(src) == "table", "First argument must be a table; got %q", src)
	Debug.Assert(type(template) == "table", "Second argument must be a table; got %q", template)
...
local function Map(t: Table, f: MapPredicate): Table
	Debug.Assert(type(t) == "table", "First argument must be a table; got %q", t)
	Debug.Assert(type(f) == "function", "Second argument must be a function; got %q", f)
...
local function Filter(t: Table, f: FilterPredicate): Table
	...
	Debug.Assert(type(f) == "function", "Second argument must be a function; got %q", f)
	local length = #t
	local newT = table.create(length)
	if length > 0 then
		local n = 0
...
local function Shuffle(tbl: Table, rngOverride: Random?): Table
	Debug.Assert(type(tbl) == "table", "First argument must be a table; got %q", tbl)
...
local function Sample(tbl: Table, size: number, rngOverride: Random?): Table
	Debug.Assert(type(tbl) == "table", "First argument must be a table; got %q", tbl)
	Debug.Assert(type(size) == "number", "Second argument must be a number; got %q", size)
	local shuffled = Copy(tbl)
...
local function Flat(tbl: Table, depth: number?): Table
	local maxDepth = if type(depth) == "number" then depth else 1
...
local function Truncate(tbl: Table, len: number): Table
	return table.move(tbl, 1, len, 1, table.create(len)) :: any
end
...
local function Zip(...): (IteratorFunc, Table, any)
	Debug.Assert(select("#", ...) > 0, "Must supply at least 1 table")
