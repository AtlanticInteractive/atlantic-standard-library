...
function LogItem:Log(message, customData)
	...
	local logMessage = string.format("%s: [%s] %s", self._log._name, self._levelName, message)
	local logLevelNum = logLevels[self._levelName]
	FireAnalyticsLogEvent(logLevelNum, string.format("%s: %s", self._log._name, message), self._traceback, customData)
...
function LogItemBlank.new(...)
	return setmetatable(LogItem.new(...), LogItemBlank)
end
...
Log.LevelNames = {}
for name, num in next, Log.Level do
...
function Log.new()
	return setmetatable({
		_name = string.match(debug.info(2, "s"), "([^%.]-)$");
		_stats = {};
	}, Log)
end
...
function Log:__tostring()
	return string.format("Log<%s>", self._name)
end
...
do
	local function SetLogLevel(name)
		...
		for levelName, level in next, Log.Level do
		...
		Debug.Error("Unknown log level: %q", name)
	end

	local configType = type(config)
	Debug.Assert(configType == "table" or configType == "string", "LogConfig must return a table or a string; got %q", config)
	if configType == "string" then
		SetLogLevel(config)
	else
		if IS_STUDIO and config.Studio then
			local studioConfigType = type(config.Studio)
			Debug.Assert(studioConfigType == "table" or studioConfigType == "string", "LogConfig.Studio must be a table or a string; got %q", config.Studio)
			if studioConfigType == "string" then
				...
				if IS_SERVER then
					...
					Debug.Assert(type(studioServerLevel) == "string", "LogConfig.Studio.Server must be a string; got %q", studioServerLevel)
					SetLogLevel(studioServerLevel)
				else
					local studioClientLevel = config.Studio.Client
					Debug.Assert(type(studioClientLevel) == "string", "LogConfig.Studio.Client must be a string; got %q", studioClientLevel)
			...
			local set = false
			local setK = nil
			for k, specialConfig in next, config do
				if k == "Studio" then
					continue
				end
				...
				elseif type(specialConfig) == "table" then
					...
					if not fallthrough then
						Debug.Assert(not set, string.format("More than one LogConfig mapping matched (%s and %s)", setK or "", k or ""))
					end

					if canUse then
						if IS_SERVER then
							local serverLevel = specialConfig.Server
							Debug.Assert(type(serverLevel) == "string", "LogConfig." .. k .. ".Server must be a string; got %q", serverLevel)
							SetLogLevel(serverLevel)
							set = true
							setK = k
						else
							local clientLevel = specialConfig.Client
							Debug.Assert(type(clientLevel) == "string", "LogConfig." .. k .. ".Client must be a string; got %q", clientLevel)
					...
					Debug.Warn("LogConfig." .. k .. " must be a table or a string; got %q", specialConfig)
				end
			end
	...
	Debug.Assert(type(logLevel) == "number", "LogLevel failed to be determined")
	if IS_STUDIO then
		local attr = IS_SERVER and "LogLevel" or "LogLevelClient"
