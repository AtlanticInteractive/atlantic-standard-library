...
local KEY_ACTIVE_EXTENSIONS = Symbol.new("ActiveExtensions")
local KEY_STARTED = Symbol.new("Started")
...
function Component.new(config: ComponentConfig)
	...
	customComponent.Started = Signal.new(customComponent[KEY_JANITOR])
	customComponent.Stopped = Signal.new(customComponent[KEY_JANITOR])
...
function Component:_setup()
	...
	local function StartComponent(component)
		...
		local hasHeartbeatUpdate = type(component.HeartbeatUpdate) == "function"
		local hasSteppedUpdate = type(component.SteppedUpdate) == "function"
		local hasRenderSteppedUpdate = type(component.RenderSteppedUpdate) == "function"
	...
	local function StartWatchingInstance(instance)
		...
		local ancestryChangedHandle = self[KEY_JANITOR]:Add(instance.AncestryChanged:Connect(function(_, parent)
