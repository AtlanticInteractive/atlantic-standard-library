...
function ClientRemoteProperty.new(re: RemoteEvent, inboundMiddleware: Types.ClientMiddleware?, outboundMiddleware: Types.ClientMiddleware?)
	local self = setmetatable({}, ClientRemoteProperty)
	self._rs = ClientRemoteSignal.new(re, inboundMiddleware, outboundMiddleware)
	self._ready = false
	self._value = nil
	self.Changed = Signal.new()
	self._readyPromise = self:OnReady():Then(function()
		self._readyPromise = nil
		self.Changed:Fire(self._value)
		self._changed = self._rs:Connect(function(value)
			value = Ser.DeserializeArgsAndUnpack(value)
...
function ClientRemoteProperty:OnReady()
	...
	return Promise.FromEvent(self._rs, function(value)
		self._value = Ser.DeserializeArgsAndUnpack(value)
		self._ready = true
		return true
	end):Then(function()
		return self._value
...
function ClientRemoteProperty:Observe(observer: (any) -> ())
	local function new(...)
		observer(Ser.DeserializeArgsAndUnpack(...))
	end

	if self._ready then
		task.defer(new, self._value)
	end

	return self.Changed:Connect(new)
end
