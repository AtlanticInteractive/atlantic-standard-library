...
local StringRep = require(script.Parent.Parent.Utility.StringRep)
...
local function GetErrorData(Error, ...) -- Make sure if you don't intend to format arguments in, you do %%f instead of %f
	...
	local ModuleName =
		if not DebugInfo then
			Prefix == 1 and table.remove(Arguments, 1) or "Command bar"
		else
			Prefix == 1 and table.remove(Arguments, 1) or table.remove(string.split(DebugInfo, ".")) or "Command bar"
...
function Debug.InspectFormat(FormatString, ...)
	...
	local Success, ErrorString = pcall(string.format, (string.gsub(FormatString, "%%q", "%%s")), table.unpack(Arguments))
...
Debug.AlphabeticalOrder = Typer.AssignSignature(Typer.Table, function(Dictionary: GenericDictionary)
	local Order = {}
	local Length = 0
...
Debug.TableToString = Typer.AssignSignature(Typer.Table, Typer.OptionalBoolean, Typer.OptionalString, function(Table: GenericTable, Multiline: boolean?, TableName: string?): string
	return ConvertTableIntoString(Table, TableName, Multiline, 1, {n = 0})
end) :: (Table: GenericTable, Multiline: boolean?, TableName: string?) -> string
...
function Debug.Inspect(...: any): string
	...
	for Index = 1, VarLength do
		...
		Length += 1
		ListBuilder[Length] = string.gsub(DataType .. " " .. DataString, "^" .. DataType .. " " .. DataType, DataType, 1)
	end
