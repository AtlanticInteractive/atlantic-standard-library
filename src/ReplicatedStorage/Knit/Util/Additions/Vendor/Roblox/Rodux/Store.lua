...
local rethrowErrorReporter = {
	reportReducerError = function(_prevState, _action, errorResult)
		error(string.format("Received error: %s\n\n%s", errorResult.message, errorResult.thrownValue))
	end;

	reportUpdateError = function(_prevState, _currentState, _lastActions, errorResult)
		error(string.format("Received error: %s\n\n%s", errorResult.message, errorResult.thrownValue))
	end;
}
...
function Store.new(reducer, initialState, middlewares, errorReporter)
	...
	if middlewares ~= nil then
		for i, middleware in ipairs(middlewares) do
			assert(type(middleware) == "function", string.format("Expected the middleware ('%s') at index %d to be a function.", tostring(middleware), i))
		end
	end
...
function Store:getState()
	if self._isDispatching then
		error(string.format("You may not call store:getState() while the reducer is executing. The reducer (%s) has already received the state as an argument. Pass it down from the top reducer instead of reading it from the store.", tostring(self._reducer)))
	end
