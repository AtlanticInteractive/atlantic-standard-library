...
function Scheduler.runTasks()
	...
	for instance, propertyTable in next, propertyChanges do
		for property, value in next, propertyTable do
			if value == None then
				value = nil
			end
	...
	for callback in next, callbacks do
		callback()
	end
...
RunService:BindToRenderStep("__FusionUIScheduler", Enum.RenderPriority.Last.Value, Scheduler.runTasks)
