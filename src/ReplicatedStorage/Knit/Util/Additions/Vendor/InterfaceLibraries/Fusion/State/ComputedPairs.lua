...
function class:update(): boolean
	...
	for dependency in next, self.dependencySet do
	...
	for key, newInValue in next, newInput do
		...
		if not shouldRecalculate then
			...
			for dependency, oldValue in next, keyData.dependencyValues do
		...
		for dependency in next, keyData.dependencySet do
...
local function ComputedPairs<K, VI, VO>(inputTable: PubTypes.CanBeState<{[K]: VI}>, processor: (K, VI) -> VO, destructor: (VO) -> ()?): Types.ComputedPairs<K, VI, VO>
	-- if destructor function is not defined, use the default cleanup function
	if destructor == nil then
		destructor = cleanup :: (VO) -> ()
	end

	local inputIsState = inputTable.type == "State" and type(inputTable.Get) == "function"
