...
function class:update(): boolean
	...
	if goalValue == self._goalValue then
		...
		if newType ~= oldType then
			...
			local springPositions = table.move(springGoals, 1, numSprings, 1, table.create(numSprings, 0))
...
local function Spring<T>(goalState: PubTypes.Value<T>, speed: PubTypes.CanBeState<number>?, damping: PubTypes.CanBeState<number>?): Types.Spring<T>
	...
	local speedIsState = type(speed) == "table" and (speed :: PubTypes.StateObject<number>).type == "State"
	local dampingIsState = type(damping) == "table" and (damping :: PubTypes.StateObject<number>).type == "State"
