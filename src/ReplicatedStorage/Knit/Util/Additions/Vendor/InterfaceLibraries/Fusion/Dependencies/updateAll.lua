...
local function updateAll(ancestor: PubTypes.Dependency)
	...
	local ancestorDependentSet = ancestor.dependentSet

	-- initialise `processNow` with dependents of ancestor
	for dependent in next, ancestorDependentSet do
		processNowSize += 1
	...
	repeat
		...
		for _, member in ipairs(processNow) do
			...
			local dependentSet = (member :: any).dependentSet
			...
			if dependentSet ~= nil then
				for dependent in next, dependentSet do
					processNextSize += 1
	...
	repeat
		...
		for _, member in ipairs(processNow) do
			...
			-- FIXME: Typed Luau doesn't understand this type narrowing yet
			if didChange then
				local localMember = member :: PubTypes.Dependent & PubTypes.Dependency
				local dependentSet = localMember.dependentSet
				if dependentSet ~= nil then
					for dependent in next, dependentSet do
						-- don't add dependents that have un-updated dependencies
						local allDependenciesUpdated = true
						for dependentDependency in next, dependent.dependencySet do
