	...
	return function(propertyTable: PubTypes.PropertyTable): Instance
		...
		do
			...
			if defaultClassProps ~= nil then
				for property, value in next, defaultClassProps do
		...
		for key, value in next, propertyTable do
			...
			elseif type(key) == "table" and key.type == "Symbol" then
				-- Event handler
				if key.name == "OnEvent" then
					...
					end) or typeof(event) ~= "RBXScriptSignal" then
		...
		if children ~= nil then
			...
			local function updateCurrentlyParented()
				...
				local function recursiveAddChild(child)
					...
					elseif childType == "table" then
						...
						if child.type == "State" then
							...
							-- array of children
							for _, subChild in next, child do
				...
				for prevChild in next, prevChildren do
				...
				for _, disconnect in next, prevConnections do
					disconnect()
				end
		...
		if parent ~= nil then
			...
			if type(parent) == "table" and parent.type == "State" then
				...
				table.insert(cleanupTasks, Observer(parent):OnChange(function()
					if ref.instance == nil then
						if ENABLE_EXPERIMENTAL_GC_MODE then
							if conn.Connected then
								warn("ref is nil and instance is around!!!")
		...
		for event, callback in next, toConnect do
