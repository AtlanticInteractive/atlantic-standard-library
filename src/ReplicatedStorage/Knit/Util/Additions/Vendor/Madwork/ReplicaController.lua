...
local SETTINGS = {
	...
	LoggingEnabled = RunService:IsStudio();
}
...
ReplicaController.NewReplicaSignal = Madwork.NewScriptSignal() -- (Replica)
ReplicaController.InitialDataReceivedSignal = Madwork.NewScriptSignal() -- ()
ReplicaController.InitialDataReceived = ValueObject.new(false)

ReplicaController._replicas = {
	--[[
		[replica_id] = {
...
ReplicaController._class_listeners = {} -- {["replica_class"] = script_signal, ...}
ReplicaController._child_listeners = {} -- {[replica_id] = {listener, ...}, ...}
...
local function CreateReplicaBranch(replica_entries, created_replicas) --> created_replicas: {replica, ...}
	...
	for replica_id_string, replica_entry in next, replica_entries do
	...
	for _, replica_entry in ipairs(sorted_replica_entries) do
		...
		elseif wait_for_parent == true then
			...
			table.insert(wait_table, self)
		end

		-- Adding replica to replica list:
		Replicas[replica_id] = self
		table.insert(created_replicas, self)
		...
		if children_waiting ~= nil then
			...
			for _, child_replica in ipairs(children_waiting) do
				child_replica.Parent = self
				table.insert(self.Children, child_replica)
			end
		end
	end
	...
	if next(waiting_for_parent) ~= nil then
		...
		for parent_replica_id, child_replicas in next, waiting_for_parent do
...
function ReplicaController.RequestData() -- Call after all client controllers are loaded and before CoreReadySignal is fired
	...
	task.spawn(function() -- In case the initial rev_ReplicaRequestData signal was lost (Highly unlikely)
		...
		while true do
			task.wait(SETTINGS.RequestDataRepeat)
			if ReplicaController.InitialDataReceived.Value == true then
				break
			end
...
rev_ReplicaRequestData.OnClientEvent:Connect(function()
	ReplicaController.InitialDataReceived.Value = true
...
rev_ReplicaSetParent.OnClientEvent:Connect(function(replica_id, parent_replica_id) -- (replica_id, parent_replica_id)
	...
	if child_listener_table ~= nil then
		for _, value in ipairs(child_listener_table) do
			value(replica)
		end
	end
end)
