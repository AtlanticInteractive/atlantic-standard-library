--[=[
	Throttles execution of a function. Does both leading, and following
	@class ThrottledFunction
]=]

local TimeFunctions = require(script.Parent.Parent.Parent.Utility.TimeFunctions)
...
function ThrottledFunction:Call(...)
	...
	elseif self._nextCallTimeStamp <= TimeFunctions.TimeFunction() then
		if self._callLeading then
			-- Dispatch immediately
			self._nextCallTimeStamp = TimeFunctions.TimeFunction() + self._timeout
			self._func(...)
	...
	elseif self._callLeading or self._callTrailing then
		...
		local remainingTime = self._nextCallTimeStamp - TimeFunctions.TimeFunction()
		self._trailingValue = table.pack(...)
...
function ThrottledFunction:_dispatch()
	...
	if trailingValue then
		...
		self._trailingValue = nil
		self._func(table.unpack(trailingValue, 1, trailingValue.n))
	end
end
...
function ThrottledFunction:_configureOrError(throttleConfig)
	...
	for key, value in next, throttleConfig do
