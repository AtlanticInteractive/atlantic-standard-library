--[=[
	...
	@class CubicSplineUtility
]=]

local BinarySearchUtility = require(script.Parent.BinarySearchUtility)
local CubicTweenUtility = require(script.Parent.CubicTweenUtility)
local LinearSystemsSolverUtility = require(script.Parent.LinearSystemsSolverUtility)

local CubicSplineUtility = {}
...
function CubicSplineUtility.NewSplineNode(t, position, velocity)
	return {
		t = t;
		p = position;
		v = velocity;
...
function CubicSplineUtility.Tween(nodeList, t)
	local i0, i1 = BinarySearchUtility.SpanSearchNodes(nodeList, "t", t)
	local node0, node1 = nodeList[i0], nodeList[i1]

	if node0 and node1 then
		return CubicSplineUtility.TweenSplineNodes(node0, node1, t)
	elseif node1 then
		return CubicSplineUtility.CloneSplineNode(node1)
	elseif node0 then
		return CubicSplineUtility.CloneSplineNode(node0)
	else
		--error("CubicSplineUtility: No node to tween with")
...
function CubicSplineUtility.CloneSplineNode(node)
	return CubicSplineUtility.NewSplineNode(node.t, node.p, node.v)
end
...
function CubicSplineUtility.TweenSplineNodes(node0, node1, t)
	local t0, t1 = node0.t, node1.t
	local p0, p1 = node0.p, node1.p
	local v0, v1 = node0.v, node1.v

	local a0, a1, a2, a3 = CubicTweenUtility.GetConstants(t1 - t0, t - t0)
	local b0, b1, b2, b3 = CubicTweenUtility.GetDerivativeConstants(t1 - t0, t - t0)

	local p = CubicTweenUtility.ApplyConstants(a0, a1, a2, a3, p0, v0, p1, v1)
	local v = CubicTweenUtility.ApplyConstants(b0, b1, b2, b3, p0, v0, p1, v1)

	return CubicSplineUtility.NewSplineNode(t, p, v)
end
...
function CubicSplineUtility.Sort(nodeList)
...
function CubicSplineUtility.PopulateVelocities(nodeList, i0, i1)
	...
	local solution = LinearSystemsSolverUtility.SolveTridiagonal(mainDiag, upperDiag, lowerDiag, output)
...
table.freeze(CubicSplineUtility)
return CubicSplineUtility
